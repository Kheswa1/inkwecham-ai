...
const ratios = [
  ...
  { label: 'Square 1:1', value: '1024:1024' },
];
...
export default function Home() {
  ...
  useEffect(() => {
    try { setHistory(JSON.parse(localStorage.getItem('video-generator-history') || '[]')); } catch {}
  }, []);
  ...
  return (
    <main className="shell">
      <section className="card hero">
        <div className="eyebrow">INKWECHAM · VIDEO GENERATOR V1</div>
        <h1>Turn a prompt into a video.</h1>
        <p className="sub">Lean text-to-video and image-to-video generation powered by Runway.</p>
        <form onSubmit={generate}>
          <label>Prompt</label>
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="A cinematic shot of a black sports car driving through Johannesburg at night..." rows={5} disabled={busy} />
        ...
        {status !== 'idle' && <div className="status"><strong>Status:</strong> {status}{taskId && <small> · {taskId}</small>}</div>}
